...
_META_HEADER_SYMBOL = '"\\\n'
_META_HEADER = """
This file is automatically generated by PymLauncher.
...
def get_launcher_meta():
    ...
    if os.path.isfile(paths.profiles_meta_file):
        with open(paths.profiles_meta_file, "r") as f:
            txt = f.read()
        if _META_HEADER_SYMBOL in txt:
            txt = txt[txt.rindex(_META_HEADER_SYMBOL) + 2 :]
        try:
            meta = json.loads(txt)
...
def load_launcher_profiles():
    ...
    if os.path.isfile(paths.profiles_file):
        ...
        try:
            ...
            for key in profile_order:
                val = profs_raw[key]
                profs[key] = LaunchProfile.from_storage(val, key)
                match profs[key].type:
                    case "latest-release":
            ...
            if not has_latest_profile:
                ...
                profs[uid] = LaunchProfile(
                    name="", type="latest-release", version_id="latest-release"
                )
