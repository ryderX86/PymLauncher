...
from PySide6.QtWidgets import (
    ...
    QCheckBox, QMessageBox, QLineEdit, QFileDialog, QBoxLayout, QGridLayout
)
...
class ProfileExporter(QThread):
    ...
    step = Signal(str, int)
    def __init__(self, parent, destination, prof:PortableProfile,
                 includes:dict={}):
    ...
    def _step(self, msg:str, max:int):
...
class ExportProfileDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        include_options.setChecked(self.profile.options_txt
                                   and self._includes["options_txt"])
        ...
        include_rps.setChecked(self.profile.resource_packs
                               and self._includes["resource_packs"])
        ...
        include_ss.setChecked(self.profile.screenshots
                              and self._includes["screenshots"])
        ...
        include_vers.setChecked(self.profile.versions
                                and self._includes["versions"])
        ...
        include_config.setChecked(self.profile.config
                                  and self._includes["config"])
        ...
        include_saves.setChecked(self.profile.saves
                                 and self._includes["saves"])
