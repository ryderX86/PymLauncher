...
from PySide6.QtCore import (
    QMimeData, QModelIndex, QPersistentModelIndex, Qt, QAbstractTableModel,
...
class ProfileModel(QAbstractTableModel):
    ...
    def canDropMimeData(self, data:QMimeData, action:Qt.DropAction, row:int,
                        column:int, parent:QModelIndex|QPersistentModelIndex):
    ...
    def data(self, index, role:int=Qt.ItemDataRole.DisplayRole):
        ...
        if (role == Qt.ItemDataRole.DisplayRole
            ...
            if index.column() == 8 and profile.has_custom_icon():
                return "<CUSTOM>"
            return [
                profile.name, profile.version_id, profile.game_dir,
                profile.java_path, jvm_args, profile.memory_min,
                profile.memory_max, res, profile.icon, profile.mods_folder
            ][index.column()]
    ...
    def _handle_del_prof(self, id:str, row:int):
