...
from pymlauncher.back.profile_manager import (
    LaunchProfile,
    save_single_profile,
)
...
from pymlauncher.front.qt.widgets import (
    Header1,
    Header2,
    IconPickerButton,
)
...
from pymlauncher.front.window.profile_exporting.export_profile import (
    ExportProfileDialog,
)
...
class ProfilesPage(QWidget):
    ...
    def _populate_resolution_combo(self):
        ...
        resolution_list = [
            str(w) + "x" + str(h) for w, h in COMMON_RESOLUTIONS
        ]
        screen = self.screen()
        geo = screen.geometry()
        if screen:
